...
./utils/batch_mark.sh assignments.txt --model gpt-5.1 --auto-approve
```
...
The model registry maps model names to their providers, enabling automatic provider resolution:

```yaml
# configs/models.yaml
defaults:
  claude:           # Default model for claude provider (optional)
  gemini:           # Default model for gemini provider (optional)
...
- When you specify `--model gemini-2.5-pro`, the system looks up the provider automatically
...
| `models.yaml` | `configs/` | Optional | Model-to-provider mappings for auto-resolution |
...
```bash
# Using the shell wrapper (recommended) - provider auto-resolved from model name
./utils/create_overview.sh <notebook_path> --model <model_name>

# Or specify provider explicitly
./utils/create_overview.sh <notebook_path> --provider <provider>
```

**Examples**:

```bash
# Using Claude (provider auto-resolved)
./utils/create_overview.sh assignments/lab1/notebook.ipynb --model claude-sonnet-4-5

# Using Gemini
./utils/create_overview.sh assignments/lab2/notebook.ipynb --model gemini-2.5-pro

# Using Codex/OpenAI
./utils/create_overview.sh assignments/lab3/notebook.ipynb --model gpt-5.1

# Or just specify provider for default model
./utils/create_overview.sh assignments/lab4/notebook.ipynb --provider claude
```
...
- Either `--model` or `--provider` is required (provider auto-resolved from model name)
...
### Feedback Summarizer (`utils/summarize_feedback.sh`)

Add feedback summaries to a grades CSV file. Copies the entire input file and adds a "Feedback Summary" column:

```bash
# Add summaries to a filled gradebook (provider auto-resolved from model)
./utils/summarize_feedback.sh section1_filled.csv --model gemini-2.5-pro

# Specify total marks if not 100
./utils/summarize_feedback.sh grades.csv --model claude-sonnet-4 --total-marks 50

# Specify custom output file
./utils/summarize_feedback.sh grades.csv --model gpt-5.1 --output final_grades.csv
...
### Feedback Modifier (`utils/modify_feedback.sh`)

Apply targeted modifications to feedback in CSV files:

```bash
# Remove specific content from all feedback (provider auto-resolved from model)
./utils/modify_feedback.sh grades.csv -i "Remove all mentions of random_state" --model claude-sonnet-4

# Add a note to all feedback
./utils/modify_feedback.sh grades.csv -i "Add 'Late submission: -10%' at the end" --model gemini-2.5-flash
...
Creates `overview.md` template for new assignments by analyzing the base notebook:

```bash
# Provider auto-resolved from model name
./utils/create_overview.sh assignments/new-assignment/notebook.ipynb --model claude-sonnet-4

# Or specify provider for default model
./utils/create_overview.sh assignments/new-assignment/notebook.ipynb --provider gemini
```

See [Generating overview.md Automatically](#generating-overviewmd-automatically) for detailed usage.
...
```bash
# Run the complete workflow (provider auto-resolved from model name)
./utils/batch_mark.sh my_assignments.txt --model gemini-2.5-pro

# Or specify provider for default model
./utils/batch_mark.sh my_assignments.txt --provider claude
```
...
```bash
# Specify model (provider auto-resolved)
./utils/batch_mark.sh assignments.txt --model claude-sonnet-4
...
./utils/batch_mark.sh assignments.txt --model gemini-2.5-pro --parallel 8

# Start fresh (ignore previous progress within each assignment)
./utils/batch_mark.sh assignments.txt --model gemini-2.5-pro --no-resume
...
- `--provider <provider>` - LLM provider (default: from overview.md, auto-resolved from model)
- `--model <model>` - Specific model (provider auto-resolved from model name)
